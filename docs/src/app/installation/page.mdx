...
### Public package channels (npm / Homebrew / Cargo)
...
### Eidos-AGI Surfari (GitHub release / web channel)

If you install from **this fork’s GitHub releases** (product name `surfari`, release asset still named `agent-browser-*`), **do not use npm** and **do not rely on the built-in `upgrade` alone** — it only knows npm/Homebrew/Cargo (and Homebrew core’s `agent-browser` is the vercel package).

Use the path documented in [Eidos install & upgrade](/eidos-upgrade):

```bash
# macOS Homebrew (Eidos tap)
...
surfari upgrade                 # re-download latest release asset from GitHub
bash scripts/install-eidos-surfari.sh --self-test   # clean-room proof
```

Also see [`docs/EIDOS_UPGRADE.md`](https://github.com/eidos-agi/surfari/blob/main/docs/EIDOS_UPGRADE.md) and the tap [`eidos-agi/homebrew-tap`](https://github.com/eidos-agi/homebrew-tap).
