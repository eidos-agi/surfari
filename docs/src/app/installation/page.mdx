...
If you install from **this fork’s GitHub releases** (product name `surfari`, release asset still named `agent-browser-*`), **do not use npm** and **do not rely on the built-in `upgrade` alone** — it only knows npm/Homebrew/Cargo.

Use the web installer and upgrade path documented in [Eidos install & upgrade](/eidos-upgrade):
...
Also see [`docs/EIDOS_UPGRADE.md`](https://github.com/eidos-agi/surfari/blob/main/docs/EIDOS_UPGRADE.md) in the repo.
